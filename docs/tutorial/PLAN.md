...
- `Job.clone_of: int|None`, `Job.session_key: str` — additive 컬럼.
  `POST /api/jobs/{vid}/practice-session` → 기준 Job의 세그먼트 전체를
  새 Job 행(같은 video_id, clone_of=기준 id, session_key=클라이언트 UUID,
  practice=True)으로 깊은 복사. 이미 그 session_key의 복제본이 있으면 재사용.
- **API 스코프**: 세그먼트 계열 엔드포인트에 선택 파라미터 `ps=<session_key>`
  — video_id→Job 해석 헬퍼가 ps 있으면 그 세션의 복제본을 반환. 프론트는
  연습 영상일 때 `localStorage jamak.practiceKey`(브라우저별 UUID)를 모든
  호출에 자동 첨부(api.ts 공통 헬퍼).
