...
- `PracticeSnapshot(job_id PK, course_id, payload JSON, source_rev, created_at)`
  — practice_course 바인딩 시(리허설 통과 상태에서) 그 Job의 세그먼트 전체를
  JSON 스냅샷. 생명주기(§10 M2):
  - `source_rev` = 그 시점 SttBlob/세그먼트 원본 해시. **재인식(세그먼트
    삭제·재삽입)이나 재바인딩 시 기존 스냅샷 무효화 + 재촬영** — 낡은
    스냅샷으로 복원 불가.
  - 코스5 결함 주입은 **원본 baseline에서 계산하는 순수 함수** — 스냅샷
    재촬영 때마다 같은 입력이면 같은 결함(멱등). 다른 코스로 재바인딩하면
    주입 없는 새 스냅샷.
- **연습 Job은 ko 단일 트랙 계약**: 번역 Translation/포크 트랙이 존재하면
  스냅샷·reset 거부(합성 튜토리얼 영상에 번역이 있을 이유 없음 — FK 복원
  계약을 만들지 않고 원천 봉쇄, §10 M3).
- `POST /api/jobs/{vid}/practice-reset` — 스냅샷에서 세그먼트 복원.
  가드(§10 M3):
  - practice Job + 유효한 스냅샷(source_rev 일치)일 때만; 일반 Job이면 403
    (BR-DATA-001 방어).
  - 프론트는 reset 전 **저장 큐 flush·취소 + undo 스택 초기화**(늦게 도착한
    PUT·이전 undo가 baseline 재오염하는 것 방지).
- 프론트: 연습 영상에서 코스 시작 시 "처음 상태로 되돌리고 시작할까요?"
  1버튼 확인 → 초기화 후 시작. 배너에도 "처음부터 다시" 버튼.
- **동시 연습 소프트 가드**(§10 B1 — lease 테이블은 과설계로 기각):
  reset API가 최근 10분 내 세그먼트 `updated_at` 변화를 감지하면 409 +
  "다른 분이 연습 중인 것 같아요" 확인 후 강행 가능. 운영 규칙: 온보딩은
  코스당 동시 1명(파일럿·순차 안내).
...
- [ ] practice-reset: 편집 → 초기화 → 스냅샷과 세그먼트 텍스트/타이밍 일치.
      일반 Job에 호출 시 403. **경합 E2E**: 저장 큐에 PUT 대기 중 reset →
      flush 후 reset이라 baseline 오염 없음; reset 후 Alt+Z 무동작(스택 비움).
...
| ID | 처리 | 내용 |
|---|---|---|
| B1 동시 충돌 lease/409 | **부분 수용** | lease 테이블은 과설계로 기각. reset API 소프트 가드(최근 10분 편집 감지 → 409+확인 강행) + 운영 규칙 "코스당 동시 1명" (§4.3) |
