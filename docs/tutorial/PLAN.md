...
- **대본 6개** (`docs/tutorial/scripts/연습N-*.md`): 코스별 전용 영상 대본,
  총 **94대사** (2026-07-17 재설계: 연습3 안심 확인 제거·찾기바꾸기 리드, 연습5 자동
  정리 제거·영상 밑 편집 바, 연습6 실전 캡스톤 전면 교체 → 79→94.
  2026-07-14 반영분: 컨트롤의 화면 위치 안내,
...
| 단계 | 담당 | 산출물 | 예상 규모 |
|---|---|---|---|
| P1 | 완료 | scripts/연습1~6.md (총 94대사 — 2026-07-17 재설계) | — |
...
- 파싱 규칙: `|`로 split, 헤더/구분행 무시, 스타일 ∈ {보통, 빠르게, 느리게, 웅얼, 침묵}.
  ...
  (조용한 스킵 금지). 파싱 총 대사 수는 `parse_scripts.EXPECTED_TOTAL`(현재 94)과
  assert — 대본을 고치면 이 상수도 같이 갱신한다 (가드가 실제로 잡아냄).
...
- [ ] 파싱 대사 수 == EXPECTED_TOTAL(94) assert.
