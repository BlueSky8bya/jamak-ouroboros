...
## v2 확장 (2026-07-15, 사용자 파일럿 피드백 — Done)

- 흐름형 확인 단계(untilTime): 체크포인트까지 나온 자막 전부 확인해야 진행,
  말풍선이 미확인 행을 따라가며 "남은 N개" 표시.
- 코스 프리셋(COURSE_PRESETS): 시작 시 재생 설정 강제 + 나레이션이 설명.
  연습 중 follow는 localStorage 저장 안 함.
- 오타 셀 지목(targetDefect): 주입 결함 행에 말풍선 고정, 이미 열려 있으면
  열기 단계 자동 통과(파일럿에서 발견한 막다른 골목).
- 대본 v2(연습1 전면 + 2·3·4 부분), EXPECTED_TOTAL 79, visuals·체크포인트 재산출,
  mp4 6개 재렌더(전부 probe 통과). 실브라우저 E2E: CHG-033 참조.
- 남은 것: 사용자 유튜브 재업로드 → 재처리·재바인딩 → 파일럿 재주행.
