...
2. **사용자 실기 확인** — ① **영상을 클릭한 뒤** `Alt+←/→`가 이탈 없이 3초 이동하는지(v0.9.84 포커스 회수 수정 검증) ② 편집칸 안 `Ctrl+←/→` 낱말 점프 실동작(CDP가 네이티브 편집 커맨드 못 만듦).
   - 파일럿에서 터진 "Alt+←가 크롬 뒤로가기" 건은 **키 선택이 아니라 유튜브 iframe 포커스 도둑**이 원인이었음(v0.9.84로 수정, ADR-0014에 기록). 영상 클릭 시 **모든** 단축키가 죽던 선행 버그.
