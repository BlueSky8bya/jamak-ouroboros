...
Last Updated: 2026-07-17 (v0.9.80 — **ADR-0014 조합키 분리: Alt=이동 / Ctrl=글자**. 탐색을 Ctrl→Alt로 옮겨 낱말 점프 복원 + 편집칸 안 탐색 둘 다 확보, 나레이션이 "알트와 화살표는 모두 이동" 한 규칙으로. v0.9.78 — 중복 셀 뿌리(이웃 흡수 가드에 퍼지 포함 폴백) + 자막 글 클릭=커서. v0.9.77 — 깻잎(오타 앵커 고정)·Alt+Z(flush 후 캡처) 브라우저 재현으로 격파)
...
2. ~~Alt+화살표 실기 확인~~ **완료**(2026-07-17, 사용자): 영상 클릭 후에도 이탈 없이 3초 이동. **`preventDefault`가 크롬 뒤로가기를 이긴다**는 것이 실기로 확인됨(개발 환경에선 검증 불가였던 항목). 파일럿에서 터진 건은 키 선택이 아니라 **유튜브 iframe 포커스 도둑**이 원인 — 영상 클릭 시 **모든** 단축키가 죽던 선행 버그였고 v0.9.84로 해소(ADR-0014 기록).
   - 남은 미검증 1건: 편집칸 안 `Ctrl+←/→` 낱말 점프 실동작(CDP가 네이티브 편집 커맨드 못 만듦 — 이벤트가 `defaultPrevented=false`로 통과하는 것까지만 확인).
