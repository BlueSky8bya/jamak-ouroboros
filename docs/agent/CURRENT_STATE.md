...
Last Updated: 2026-07-18 (v0.9.100 — **漢 발굴 병기 경합 방지**(리뷰 F2): 한 셀에 사전 병기(fire-forget queueSave)+발굴(promote)이 함께면 순서 역전 시 발굴 병기 소실 가능 → promote 직전 겹치는 셀 저장 await로 순서 고정. picks-only 불변. 스크래치 재검증 통과. v0.9.99 — **ADR-0016 P2-c 발굴 후보 선택 UI 완료**: 漢 채우기 모달에 `.disc-section` — 발굴 한자어를 후보 라디오(추천+동음+무시)로 띄우고 관리자가 고른 것만 `/promote-hanja`로 확정(병기+등록). 한 셀 발굴 여럿 충돌하던 **복합키 버그**(discPick·라디오 name을 segment_id만으로 키잉) → `discKey=segment_id\x1freading` 수정. 스크래치 검증: "무상과 식을 알라" 발굴 2행 독립, 식 識→式 바꿔도 무상=無常 유지, 적용 후 클론 셀 `무상(無常)과 식(式)을 알라` + HanjaTerm 無常·式 등록. **다음: P2-e 실API 발굴 검증(관리자·유료) + 연습6 漢 스텝 재렌더 제거**. v0.9.98 — ADR-0016 P2 백엔드: 맞춤법 사전밖 한자어 발굴(discovered/alt/gloss) + fill_hanja가 discoveries 반환 + /promote-hanja 검증·등록. 자동병기 금지=환각방지. v0.9.97 — 단일자 한자사전 오병기 청소: special 382→177(207 강등, 貪嗔癡 유지) + 힌트 목록 count>=4 하한(258종→41종). v0.9.96 — **漢 채우기 관리자 전용화**(ADR-0016 P1: 서버 _require_admin + 프론트 isAdmin). ADR-0016 확정 — 사전 밖 한자어 문맥 발굴 + 관리자 후보 검증 + 학습 승격(P2, ACTIVE_PLAN). **자동 병기 절대 금지=환각 방지**. 파급: 연습6 漢 스텝 재렌더 제거 필요. v0.9.95 — **ADR-0015 한자 병기 힌트**: 맞춤법 API가 문맥으로 special 단독 1글자(貪·嗔·癡) 병기 후보를 기록(텍스트 불변) → Segment.hanja_hints → 漢 채우기 규칙 C가 무-API 소비. 십악중죄 삼독 수동 등록. NOT VERIFIED: 관리자가 실영상에서 맞춤법→漢 흐름 1회 필요. v0.9.94 — 연습5 자막 "0.1초"(음성·렌더 무관: 셀 텍스트는 practice.py, 음성은 대본 — v0.9.92에서 분리됨). Alt+[ 건은 **포커스 버그가 아니라 가시성 버그**로 판명·해결. v0.9.91~93 — **연습5 셀을 대본 문장 기준 재구성**(텍스트는 대본 원문, 시각은 stt words에 difflib 정렬 → 재렌더 강건): 죽어 있던 빠른 문장 드릴 복구(cps 9.4 → 22, 빨간 표시). 무음 다듬기 스텝 순서가 대본과 반대였던 것 수정. 결함 주입 멱등성 버그(재바인딩마다 +1) 수정. 포커스 셀 화면으로 스크롤. **미해결: Alt+[ 단계 포커스가 엉뚱한 셀 → 투어대로 누르면 그 셀 타이밍이 파괴됨(최우선, 판별 관찰 대기)**. v0.9.88~89 — **`visuals.ts`가 옛 대본에 묶여 죽은 버튼(안심 확인·자동 정리·복구·채우기)을 영상이 안내**하고 있었음. 3·5·6 매핑 재작성 + 재렌더 + 재바인딩 완료(1·2·4는 대본 불변이라 이상 없음, 감사 도구로 6코스 전수 확인). 찾기·바꾸기 드릴 4행 주입. v0.9.85~87 — **중복 셀의 진짜 뿌리 = `split.py`**: 어절 한가운데 절단(`0.75를`) + 뜸들인 한 낱말 절단(`제`) → 조각 셀 → LLM이 문맥으로 확장 → 이웃과 중복. 판정은 `_SENT_END`(부호 또는 종결어미)로, 실측 136개 절단점이 갭·길이·부호 기준을 전부 기각. 연습2·3 재처리 완료. 찾기·바꾸기 스텝이 실제 바꾸기를 요구하도록 연쇄 2단계. v0.9.84 — 유튜브 iframe 포커스 도둑(영상 클릭 시 **모든** 단축키 사망) 회수. v0.9.80 — ADR-0014 Alt=이동 / Ctrl=글자)

## 지금 대기 중인 것 (다음 세션 우선)

1. ~~연습2 재렌더·업로드·재바인딩~~ **완료**(v0.9.83) — playback = `VxMrmcjtnIA`(31셀). ADR-0014가 앱·나레이션·영상 화면·체크포인트 전부 일치 상태로 완결. **다음 파일럿 확인 대상**: 연습2를 실제로 돌려 Alt 키 흐름이 매끄러운지.
2. ~~Alt+화살표 실기 확인~~ **완료**(2026-07-17, 사용자): 영상 클릭 후에도 이탈 없이 3초 이동. **`preventDefault`가 크롬 뒤로가기를 이긴다**는 것이 실기로 확인됨(개발 환경에선 검증 불가였던 항목). 파일럿에서 터진 건은 키 선택이 아니라 **유튜브 iframe 포커스 도둑**이 원인 — 영상 클릭 시 **모든** 단축키가 죽던 선행 버그였고 v0.9.84로 해소(ADR-0014 기록).

0. ~~연습5 `Alt+[`가 엉뚱한 셀을 고침~~ **해결**(v0.9.93, 사용자 재확인). 판별 관찰 결과 **포커스는 처음부터 옳았다** — 영상이 되감겨 빠른 문장을 들려주고 `.timing-bar`가 `#10`(빠른 문장 셀)을 가리킨다. 문제는 **그 셀이 화면 밖이라 무엇을 고치는지 볼 수 없었던 것**. 데이터가 깨지고 있던 게 아니라 "깨질 것처럼 보였던" 것이고, 그 상태로 어르신에게 줬으면 실제 사고가 났을 상황. `focusSegment`가 셀을 화면 가운데로 끌어오게 해 해소.
   - 교훈: 사용자가 말한 증상("저 위 셀이 파괴된다")은 **추론**이었고 실제 원인은 가시성이었다. 증상을 그대로 믿고 포커스 로직을 뜯었으면 멀쩡한 코드를 망칠 뻔했다 — 판별 관찰(되감기 여부 + `#번호`) 하나가 갈랐다.

3. **사용자 실기 확인 대기**:
   - 연습3 찾기·바꾸기(v0.9.87) — 창만 열면 **안** 넘어가고, `몽치`→`뭉치` 바꾸기를 실행해야 넘어가는지. (드릴 재료는 검증 완료: 새 영상에 `몽치` 4행 주입, 한 번에 4개 고쳐짐)
   - 편집칸 안 `Ctrl+←/→` 낱말 점프 실동작(CDP가 네이티브 편집 커맨드 못 만듦 — 이벤트가 `defaultPrevented=false`로 통과하는 것까지만 확인).
   - 연습3·5·6 새 영상(v0.9.89) 한 바퀴 — 죽은 버튼 안내 사라졌는지(스틸로는 확인).

6. ~~final 체크포인트 규칙 혼재~~ **해결**(v0.9.90, 사용자 확정 "시작으로 통일"). 재계산해 보니 "혼재"가 아니라 **회귀**였다 — 원래 규칙은 처음부터 `start+0.15`였고(`basic`·`playback`·`structure`는 지금도 그랬음), v0.9.70에서 3·5·6 값을 갱신하며 `end` 기준으로 잘못 넣어 그 셋만 어긋난 것. 🎉가 마무리 말 끝난 뒤 뜨던 것(fast 11.3s·timing 9.1s·finish 18.7s 지연) 복구. 규칙은 `tutorialSync.ts` 헤더 주석에 명시.

4. **고칠 수 없음(설명 완료)**: 주소창에 포커스가 있을 때 `Alt+↑/↓`가 크롬 자동완성으로 새는 것 — 키 입력이 브라우저 UI로 가고 페이지엔 도달하지 않아 `preventDefault`를 걸 자리가 없다. 화면 한 번 클릭이 해법.

5. **분할 수정의 잔여 영향**: 기존 영상들엔 얇은 조각 셀 **240건**·어절 중간 절단 **6건**이 남아 있다(v0.9.85~86은 **미래 처리분**에만 적용). 실영상은 검수 과정에서 자연 해소 예상 — 소급 재처리는 범위 밖. 연습 영상만 재처리 완료(2·3).
...
- **검수자(비관리자)**: API 안 쓰는 대부분 — 타이밍 자동정리·무음 다듬기·나누기/합치기·찾기바꾸기·🙉·되돌리기·확인, **그리고 번역 검수(기존 번역 보기·수정)**.
- **관리자(운영자)만**: Claude API 쓰는 **생성** 동작 — 맞춤법 검사·**번역 생성/재번역**·초안 생성. (번역 *검수*는 검수자에게 열림 — 검수엔 API 안 씀.) **+ 漢 채우기**(무-API지만 ADR-0016으로 관리자 전용화 — 발굴 후보 검증이 환각 방지 게이트라 사람 판단 필수).
