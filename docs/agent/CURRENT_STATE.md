...
Last Updated: 2026-07-17 (v0.9.85~87 — **중복 셀의 진짜 뿌리 = `split.py`**: 어절 한가운데 절단(`0.75를`) + 뜸들인 한 낱말 절단(`제`) → 조각 셀 → LLM이 문맥으로 확장 → 이웃과 중복. 판정은 `_SENT_END`(부호 또는 종결어미)로, 실측 136개 절단점이 갭·길이·부호 기준을 전부 기각. 연습2·3 재처리 완료. 찾기·바꾸기 스텝이 실제 바꾸기를 요구하도록 연쇄 2단계. v0.9.84 — 유튜브 iframe 포커스 도둑(영상 클릭 시 **모든** 단축키 사망) 회수. v0.9.80 — ADR-0014 Alt=이동 / Ctrl=글자)
...
3. **사용자 실기 확인 대기** (v0.9.87, 브라우저 E2E 실패로 미검증):
   - 연습3 찾기·바꾸기 — 창만 열면 **안** 넘어가고, `몽치`→`뭉치` 바꾸기를 실행해야 넘어가는지.
