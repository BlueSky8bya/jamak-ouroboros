...
- **PENDING (승인 대기)**: ① 새 기준본 4개(연습 2·4·5·6)의 기존 중복 자막 13행 데이터 정리 — 가드는 앞으로의 실행만 막고 이미 들어간 행은 남음. 정리 스크립트 준비됨(`dup_fix_v2.py` — 삭제 4행·축소 9행·클론 폐기), 운영 DB 쓰기라 사용자 승인 대기. ② 구 practice Job 6개 DB 삭제(유튜브에서 이미 삭제됨, "코스 없는 자유 연습" 섹션에 잔존) — 동일 승인 대기.
