...
- **PENDING (승인 대기)**: ① 새 기준본 4개(연습 2·4·5·6)에서 중복 자막 13행 발견 — LLM 교정의 규칙 9 위반(YT 완전 문장으로 확장, 9건) + 에코 가드 우회(STT 오타로 매칭 실패, 4건). 정리 스크립트 준비됨(`dup_fix_v2.py` — 삭제 4행·축소 9행·클론 폐기), 운영 DB 쓰기라 사용자 승인 대기. ② 구 practice Job 6개 DB 삭제(유튜브에서 이미 삭제됨, "코스 없는 자유 연습" 섹션에 잔존) — 동일 승인 대기.
