...
1. **연습2 재렌더** (ADR-0014, 사용자 승인 완료) — 대본·키캡은 이미 고쳐 커밋됨(`연습2-재생다루기.md` L5·L7·L9·L12, `visuals.ts`). 남은 순서: `tools/tutorial-video` 렌더 → 사용자가 유튜브 업로드 → 새 `video_id`로 playback 코스 재바인딩 → **렌더 산출 `out/practice-2/timing.json` 기준으로 `TUTORIAL_CHECKPOINTS.playback` 갱신**(대사 길이가 "컨트롤"→"알트"로 줄고 L12에 문장이 늘어 기존 값 무효).
