...
0. 🔴 **최우선 — 연습5 `Alt+[` 단계가 엉뚱한 셀을 고침**: 투어가 시킨 대로 눌렀는데 **다른 셀의 시작점이 파괴**된다(사용자 실기). 튜토리얼이 데이터를 망가뜨리는 것이라 다른 무엇보다 먼저. 정적으로는 `subject [51.4,57.6]` → #9(52.3~54.3) 정확히 집고 `tourEnterStep`이 `focusSegment(#9)`를 호출하는 게 맞는데 증상이 다르다 — **판별 관찰 대기**: ① 말풍선 뜰 때 영상이 52초로 되감겨 빠른 문장을 다시 들려주는가(=`tourEnterStep` 실행 여부) ② `.timing-bar`의 `#번호`. v0.9.93은 포커스 스크롤만 고쳤을 뿐 **포커스 대상 자체는 미해결**.
