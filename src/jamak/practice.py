...
TIMING_SCRIPT: list[str] = [
    ...
    # 자막은 숫자로 적는다 — 말은 "영점 일 초"라고 읽어도 읽는 글은 "0.1초"가
    # 눈에 빨리 들어온다(사용자 지적). 나레이션 대본(docs/tutorial/scripts/)은
    # 그대로 두므로 **음성·재렌더와 무관**하다. 정렬은 STT가 들은 "영점일"과
    # 이 "0.1"이 어긋나지만, difflib이 앞뒤 문장으로 위치를 잡아 준다(검증함).
    "한 번 누를 때마다 0.1초씩 움직입니다.",
