...
10. **[한자 병기 후보 — 텍스트는 절대 바꾸지 않는다]** 아래 '강조 한자어' 목록의 낱말이 이 줄에 **바로 그 뜻으로** 쓰였으면, 텍스트는 그대로 두고 `hanja`에만 `{idx, reading(음), hanja(한자)}`로 기록한다. 규칙:
  - **fixes(텍스트)는 절대 바꾸지 않는다.** 괄호 병기를 넣지 않는다. hanja는 오직 기록이다.
  - **같은 소리 다른 뜻이면 기록하지 않는다.** 예: '탐험'·'탐구'의 '탐'은 貪(탐할 탐)이 아니다 → 기록 안 함. '십악' 같은 불교 문맥에서 홀로 나온 '탐'만 貪이다.
  - **목록에 없는 낱말은 기록하지 않는다.** 확신이 없으면 기록하지 않는다(과잉보다 미기록이 낫다).
  - reading은 목록의 음 그대로, hanja는 목록의 한자 그대로 적는다."""
